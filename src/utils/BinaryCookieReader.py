...
for page in pages:
	...
	for offset in cookie_offsets:
		...
		expiry_date=datetime.datetime.fromtimestamp(0) + datetime.timedelta(seconds=expiry_date_epoch)
		expiry_date=expiry_date.strftime("%a, %d %b %Y ")[:-1]
				
		create_date_epoch=unpack('<d',cookie.read(8))[0]+978307200           #Cookies creation time
		create_date=datetime.datetime.fromtimestamp(0) + datetime.timedelta(seconds=create_date_epoch)
		create_date=create_date.strftime("%a, %d %b %Y ")[:-1]
