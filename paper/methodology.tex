...
and ~\autoref{fig:yahoo-dom-user}. The x-axis is bits of simhash value and
y-axis is the id of each observation (id increases in the order of time).
...
compare $s_{j}$ with $S_{spider}$, and take into consideration of the all the
collected simhash, the distance from $s_{j}$ to centroid of $S_{spider}$ is a
...
Let $T_{learn}$ denote the threshold for inconsistent coefficient, by applying
~\autoref{coefficient:learn} in clustering phase, $S_{spider}$ is divided into
serveral clusters $S_{spider, k}, k \in (1, c)$. These clusters are
Simhash-based Website Model for this website.

We represent each cluster $S_{spider, k}$ with centroid and links formed in
...
compute distance from new observation to current cluster $d$, $links$ are used
to compute $\mu$ and $\sigma$.

\begin{equation}
  \label{coefficient:learn}
  \alpha = \frac{d - \mu}{\sigma} < T_{learn}
\end{equation}
...
learned clusters , which is$S_{spider, k}, k \in (1, c)$. 
